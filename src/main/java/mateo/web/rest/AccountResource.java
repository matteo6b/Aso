...
import mateo.domain.Authority;
import mateo.domain.PersistentToken;
import mateo.domain.User;
import mateo.repository.PersistentTokenRepository;
import mateo.repository.UserRepository;
import mateo.security.SecurityUtils;
import mateo.service.MailService;
import mateo.service.UserService;
import mateo.web.rest.dto.KeyAndPasswordDTO;
import mateo.web.rest.dto.UserDTO;
...
public class AccountResource {
     ...
     * @return the ResponseEntity with status 201 (Created) if the user is registred or 400 (Bad Request) if the login or e-mail is already in use
     */
    @RequestMapping(value = "/registerSocio",
        method = RequestMethod.POST,
        produces={MediaType.APPLICATION_JSON_VALUE, MediaType.TEXT_PLAIN_VALUE})
    @Timed
    public ResponseEntity<?> registerSocio(@Valid @RequestBody UserDTO userDTO, HttpServletRequest request) {
        ...
        return userRepository.findOneByLogin(userDTO.getLogin())
            ...
            .orElseGet(() -> userRepository.findOneByEmail(userDTO.getEmail())
                .map(user -> new ResponseEntity<>("e-mail address already in use", textPlainHeaders, HttpStatus.BAD_REQUEST))
                .orElseGet(() -> {
                    User user = userService.createSocioInformation(userDTO.getLogin(), userDTO.getPassword(),
                        userDTO.getFirstName(), userDTO.getLastName(), userDTO.getEmail().toLowerCase(),
                        userDTO.getLangKey());
                    String baseUrl = request.getScheme() + // "http"
                        "://" +                                // "://"
